...
/**
 * Give access to the [[com.mohiva.play.silhouette.impl.providers.OAuth1Info]] object.
 *
 * @param reactiveMongoApi The ReactiveMongo API.
 * @param config The Play configuration.
 */
final class OAuth1InfoDAO @Inject() (reactiveMongoApi: ReactiveMongoApi, config: Configuration)
  extends DelegableAuthInfoDAO[OAuth1Info] with MongoDAO {

  /**
   * The collection to use for JSON queries.
   */
  val jsonCollection = reactiveMongoApi.db[JSONCollection](
    config.getString("silhouette.persistence.reactivemongo.collection.auth.info.oauth1").getOrElse("auth.info.oauth1")
  )
  ...
  def find(loginInfo: LoginInfo) = {
    jsonCollection.find(Json.obj("_id" -> loginInfo)).projection(Json.obj("_id" -> 0)).one[OAuth1Info]
  }
   ...
   */
  def add(loginInfo: LoginInfo, authInfo: OAuth1Info) = {
   ...
   */
  def update(loginInfo: LoginInfo, authInfo: OAuth1Info) = {
    updated(jsonCollection.update(Json.obj("_id" -> loginInfo), merge(loginInfo, authInfo))).map {
      case num if num > 0 => authInfo
      case _ => throw new MongoException("Could not update OAuth1 info for login info: " + loginInfo)
    }
  }
  ...
  /**
   * Merges the [[LoginInfo]] and the [[OAuth1Info]] into one Json object.
   *
   * @param loginInfo The login info to merge.
   * @param authInfo The auth info to merge.
   * @return A Json object consisting of the [[LoginInfo]] and the [[OAuth1Info]].
   */
  private def merge(loginInfo: LoginInfo, authInfo: OAuth1Info) =
